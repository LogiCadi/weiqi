...
LineChart.prototype.touchBegin = function (event) {};

LineChart.prototype.touchEnd = function (event) {
  ...
  let step = Math.round((x - xOffset) / xStep);
  return step;
};

function LineChart(ctx, options) {
  this.ctx = uni.createCanvasContext(ctx);
  this.tipsCtx = options.tipsCtx ? uni.createCanvasContext(options.tipsCtx) : null;
...
LineChart.prototype._tipsLabelByIndex = function (index) {
  ...
  let fromY = margin;
  let endY = margin + (points.length + 1) * 18; // 背景
...
LineChart.prototype._drawAxis = function () {
  ...
  let yMaxValue = _.max(_.map(lines, item => _.max(item.points))) || 1; // 计算需要绘制的y轴label
  ...
  let yStep = yAxisLen / (yMaxValue * 1.2); // 绘制x轴
  ...
  _.each(xLabel, item => {
    let xValue = xOffset + item.index * xStep - xLabelMaxWidth / 3 - 2;
    ctx.fillStyle = labelColor;
    ctx.fillText(item.name, xValue, height - margin);
    ctx.beginPath();
    ctx.lineWidth = 0.5;
    ctx.setLineDash([8, 8]);
    ctx.moveTo(xOffset + item.index * xStep, margin);
    ctx.lineTo(xOffset + item.index * xStep, height - xAxisOffset);
    ctx.stroke();
  }); // 绘制y轴，以及水平标记线
  ...
  ctx.strokeStyle = axisColor;
  ctx.moveTo(yLabelMaxWidth, height - xAxisOffset);
  ctx.lineTo(yLabelMaxWidth, margin);
  ctx.stroke();
  let temp = height - xAxisOffset - margin;
  ...
  for (let i = 0; i < 5; i++) {
    ...
    ctx.lineWidth = 0.5;
    ctx.setLineDash([8, 8]);
    ctx.moveTo(yLabelMaxWidth, margin + i * temp / 5);
    ctx.lineTo(width - margin, margin + i * temp / 5);
    ctx.stroke();
    ctx.strokeStyle = labelColor;
    let txt = (5 - i) * 20;
    ctx.fillText(txt, yLabelMaxWidth - 2 * margin, margin + i * temp / 5 + temp / 20);
  }
  /*   // 绘制y轴label，以及水平标记线
...
LineChart.prototype._drawLine = function (line) {
  ...
  points = points.filter(function (item, index) {
    if (item.y - yOffset <= 0) {
      return item;
    }
  }); // 与x轴的面积阴影
  ...
  _.each(points, item => {
    ctx.beginPath();
    ctx.arc(item.x, item.y, 1.5, 0, 1.5 * Math.PI);
    ctx.fillStyle = line.color;
    ctx.fill(); //ctx.stroke();
