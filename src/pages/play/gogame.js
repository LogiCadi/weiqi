...
export default class WyGoGame {
  ...
  Move(coord, color) {
    ...
    uni.setStorage({
      key: gamesavedmycolorkey,
      data: this.m_nMyColor
    });
    return true;
  }
  ...
  onTimer() {
    ...
    if (this.gamemode != 2 && !this.gameisend && this.m_nTimerTimes % 10 == 0) {
  ...
  updateTitleInfo() {
    if (this.wygo != null) {
      ...
      if (this.gamemode == 5) {
        ainame = "KataGo";
      } else if (this.gamemode == 1 || this.gamemode == 0) {
        ainame = "LeelaZero";
      }

      let blackplayer = ainame;
      let whiteplayer = "Me";
  ...
  Test2() {
    uni.playVoice({
      filePath: '/gog361/images/aou.wav',
      complete: function () {}
    });
  }
  ...
  playaou() {
    ...
    audio.title = "音乐文件";
    audio.src = "/gog361/images/aou.wav";
    audio.play();
    audio = null;
  }
  ...
  playmove() {
    ...
    audio.title = "音乐文件";
    audio.src = "/gog361/images/move.wav";
    audio.play();
    audio = null;
  }
  ...
  playpass(color) {
    ...
    if (color == BLACK) {
      audio.src = "/gog361/images/pass_black.mp3";
    } else {
      audio.src = "/gog361/images/pass_white.mp3";
    }
  ...
  playwin() {
    ...
    audio.title = "音乐文件";
    audio.src = "/gog361/images/win.mp3";
    audio.play();
    audio = null;
  }
  ...
  getRandSgf() {
    var that = this;
    let theurl = "/gog361/flask/v1/joygo/randsgf/" + app.userid;
    uni.request({
      url: theurl,
      header: {
  ...
  getAINextMove() {
    ...
    let theurl = "/gog361/flask/v1/app/aiapp2";
    ...
    if (this.gamemode == 3) {
      ...
      operation = "kjoygoai";
      theurl = "/gog361/flask/v1/app/kaiapp";
    }
  ...
  get_Kata_NN_Result() {
    ...
    let theurl = "/gog361/flask/v1/app/kaiapp";
    var that = this;
    uni.request({
      url: theurl,
      method: "POST",
  ...
  save_game_record(wincolor, gameendresult) {
    ...
    if (this.wygo != null && this.wygo.GetRealStepCount() < 20) {
      ...
      if (this.gamemode == 2) {
        blackuserid = app.userid;
        whiteuserid = app.userid;
      ...
      let theurl = "/gog361/cgi-bin/listgamerecord?op=savegame&userid=" + app.userid;
      +"sessionkey=" + app.sessionkey;
      uni.request({
        url: theurl,
        method: "POST",
  ...
  get_shuzi_count() {
    ...
    var that = this;
    let theurl = "/gog361/flask/v1/app/aiapp";
    uni.request({
      url: theurl,
      method: "POST",
      header: {
  ...
  checkVip() {
    if (this.gamemode == 0 || this.gamemode == 1 || this.gamemode == 5) {
      if (app.userstar > 0) {
        return true;
      } else {
        if (this.wygo != null && this.wygo.GetRealStepCount() > 80) {
          return false;
        }
  ...
  showToBuyVIP() {
    ...
    if (res.platform == 'ios') {
      uni.showModal({
        title: '提示',
        content: '职业级人机对弈和KATA对弈，体验已经完成，需要成为会员才可以继续对弈，苹果手机请联系微信wyz785。'
      });
      return;
    }
    ...
    uni.showModal({
      title: '提示',
      content: '职业级人机对弈和KATA对弈，体验已经完成，需要成为会员才可以继续对弈',
      success: function (res) {
        if (res.confirm) {
          app.toShowBuyProduct();
          console.log('用户点击确定');
        } else if (res.cancel) {
          console.log('用户点击取消');
        }
