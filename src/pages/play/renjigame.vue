<template>
	...
	<view class="allview" catchtouchmove="preventScroll">
		...
		<view class="gmboard" :style="'top: ' + gmboard_top + 'px; height: ' + gmboard_height + 'px;'" catchtouchmove="preventScroll">
			<canvas id="myCanvas" v-if="showcanvas" :style="'width:' + canvasview.width + 'px;height:' + canvasview.height + 'px;'"
			 canvas-id="image-canvas-1" catchtouchmove="preventScroll" @mousemove="setPrePosition" @click="xiazi"></canvas>
		</view>
		...
		<view class="title_container_ex" v-if="showgameinfo" :style="'top: ' + commentview_top + 'px; height: ' + commentview_height + 'px;'">
			<view class="title_container_ex" :style="'top: ' + commentview_top + 'px;height: ' + commentview_height * .5 + 'px; right: 0;'">
			...
			<view class="title_container_ex" :style="'top: ' + commentview_top_2 + 'px; height: ' + commentview_height * .5 + 'px; right: 0;'">
		...
		<view class="functionbtnview" :style="'top: ' + functionbtnview_top + 'px;height:' + functionbtnview_height + 'px;'">
		...
		<!-- <view class="linechartview" v-if="showlinecanvas" :style="'top:' + linechart_top + 'px; height: ' + linechart_height + 'px;'"
		 catchtouchmove="preventScroll" catchtouchstart="touchLineChartBegin">
			<canvas :style="'width:' + canvasview.width + 'px; top:0px; height: 100%;'" canvas-id="mylinechart" catchtouchmove="preventScroll"
			 @touchmove="touchLineChartBegin" @touchstart="touchBegin" @touchend="touchEnd"></canvas>
		</view> -->

		<view class="bottombtnview" :style="'top: ' + bottombtnview_top + 'px; height: ' + bottombtnview_height + 'px;'">
		...
		<icon type="success" :size="movebtn_width" class="movebtn" :style="'width: ' + movebtn_width + 'px; height: ' + movebtn_width + 'px; top: ' + movebtn_top + 'px; left: ' + movebtn_left + 'px;'"
		 @tap="onClickMove" v-if="showmovebutton && !isPC"></icon>

		<view class="startgame-dialog" :style="' height: ' + startview_height + 'px;'" v-if="showstartview">
			<view class="modal-title">设置先后手和让子数</view>
			...
			<view class="modal-footer">
				<view class="btn-confirm" style="color: black; border-right: 1px solid #eee;" @tap="onStartCancel" data-status="cancel">取消</view>
	...
	export default {
		...
		methods: {
			layout(adheight) {
				...
				let movebtnwidth = 50;
				let boardSize = winwidth
				if (isPC) {
					boardSize = winheight
					gameboardtop = 0
					comment_top = 25
					comment_height = 50
					comment_width = winwidth - winheight
					bottomcommenttop = 75
					bottomcommentheight = 50
					functionbtnviewtop = 125
					functionbtnviewheight = 60
					movebtnwidth = 80
					bottombtnviewtop = winheight * .45
					bottombtnviewheight = winheight * .45
					// bottomcommentwidth = winwidth - winheight
				}
				...
				this.setData({
					...
					commentview_width: comment_width,
					...
					movebtn_left: winwidth - comment_width / 2 - movebtnwidth / 2,
			...
			initPage() {
				...
				if (typeof moves != "undefined" && moves != null && base.isArray(moves) && moves.length > 0) {
					let gamesavedmycolorkey = "renjimycolor" + this.gamemode;;
					let mycolor = uni.getStorageSync(gamesavedmycolorkey);

					if (typeof mycolor == "undefined" || mycolor == null || mycolor == "") {
					...
					this.$nextTick(function() {
						wygogame = new WyGoGame(this, this.gamemode, mycolor, this.rangzinumber, moves);
					})
				} else {
					if (this.gamemode == 1) {
						this.setData({
			...
			onStartConfirm: function() {
				...
				this.$nextTick(function() {
					if (wygogame == null) {
						...
						wygogame.init(this, this.gamemode, this.mycolor, this.rangzinumber, null);
					}
				})
			},
