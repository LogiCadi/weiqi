...
export default class WyViewGoGame {
  constructor(page, option, sgf) {
    this.init(page, option, sgf);
  }

  init(page, option, sgf) {
    ...
    this.option = option;
    this.page.setData({
      black_nick: this.option.blackplayer,
      white_nick: this.option.whiteplayer
    });
    this.gamemode = parseInt(this.option.gamemode);
    this.commentkey = "";

    if (this.gamemode == 0) {
      this.commentkey = "qipu-" + this.option.history + "-" + this.option.gamestarttime + "-" + this.option.blackuserid + "-" + this.option.whiteuserid;
    } else {
      this.option.history = 0;
      this.option.gamestarttime = 0;
      this.commentkey = "p-" + this.option.date + "-" + this.option.blackplayer + "-" + this.option.whiteplayer;
    }
  ...
  submitcommet(themsg) {
    ...
    uni.request({
      ...
      data: {
        ...
        gamestarttime: this.option.gamestarttime,
        blackuserid: this.option.blackuserid,
        whiteuserid: this.option.whiteuserid,
