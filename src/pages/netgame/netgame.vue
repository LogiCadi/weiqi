...
<view class="allview" catchtouchmove="preventScroll">
 ...
 <view class="functionbtnview" :style="'flex-direction:row; top: ' + functionbtnview_top + 'px;height:' + functionbtnview_height + 'px;'">
    ...
    <button class="functionbtn" @tap="btn_calljudge" :disabled="btn_calljudge_disabled">{{btn_calljudge_text}}</button>
   ...
   <block v-for="(item, index) in chatmsg_array" :key="index"> 
           <view class="cellview" style="flex-direction:row;">
             <view class="cellitem0" v-if="item.fromme==0">
              ...
              <text class="chatitem" v-if="item.fromme==0">{{item.FromUserNick}}:{{item.Msg}}
              </text>
              <text class="chatitemfromme" v-if="item.fromme==1">{{item.FromUserNick}}:{{item.Msg}}
              </text>
  ...
  <view class="bottombtnview" v-if="showbottombtnview==1" :style="'flex-direction:row; top: ' + bottombtnview_top + 'px; height: ' + bottombtnview_height + 'px;'">
    ...
    <button class="bottombtn" @tap="btn_exit" :disabled="btn_eixt_disabled">{{btn_exit_text}}</button>
  </view>
  <icon type="success" :size="movebtn_width" class="movebtn" :style="'width: ' + movebtn_width + 'px; height: ' + movebtn_width + 'px; top: ' + movebtn_top + 'px; left: ' + movebtn_left + 'px;'" @tap="onClickMove" v-if="showmovebutton"></icon>
   ...
   <view class="bottombtnview" v-if="showbottombtnview==0" :style="'flex-direction:row; top: ' + bottombtnview_top + 'px; height: ' + bottombtnview_height + 'px;'">
        ...
        <button class="sendbtn" @tap="onSendMsg">发送</button>
  </view>
...
export default {
  data() {
    return {
      ...
      topcommenttext: "",
      showgameinfo: true,
      btn_calljudge_disabled: false,
      btn_calljudge_text: "呼叫裁判",
      showchatview: true,
      functionbtnview_top: "",
  ...
  onShareAppMessage: function () {
    let thepath = "/pages/netgame/netgame?type=3&roomid=" + this.option.roomid + "&gamedeskid=" + this.option.gamedeskid + "&blackuserid=" + this.option.blackuserid + "&whiteuserid=" + this.option.whiteuserid + "&fromuserid=" + app.globalData.userid;
    return {
      title: '我正在进行网络对弈，欢迎观战',
      path: thepath,
      imageUrl: '/',
  ...
  methods: {
    ...
    initPage() {
      ...
      this.setData({
        ...
        fromtype: parseInt(this.option.fromtype)
      });
      ...
      wygogame = new WYNetGame(this, this.gamemode, this.fromtype);
      /*	let gamesavedkey="renji"+this.data.gamemode;
      	...
      	if(typeof(moves)!="undefined" && moves!=null && base.isArray(moves)&&moves.length>0)
      	{
      		...
      		wygogame=new WyGoGame(this,this.data.gamemode, this.data.mycolor,this.data.rangzinumber,moves);
      	}
      	else
      	{
      		if(this.data.gamemode==1)
      		{
      			this.setData({
      	            showstartview: false,
      	            showcanvas: true
            	 	 })	
      			wygogame=new WyGoGame(this,this.data.gamemode, BLACK, 0, null);
      			}
      		else
      		{
      			this.setData({
    ...
    onSendMsgBack: function () {
      wygogame.onSendMsgBack();
    },
    btn_calljudge: function () {
      wygogame.btn_calljudge();
    },
    onSendMsg: function () {
